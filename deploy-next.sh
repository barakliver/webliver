...
# The route types are generated, they belong to the build that is currently
# serving, and `tsconfig.json` includes them. That is fine going forwards and
# fatal going backwards: rolling back to a commit with fewer routes leaves
# `.next/types/validator.ts` importing pages that no longer exist in the
# checked-out source, so the type check below dies on TS2307 for every one of
# them and the rollback can never build. That is how a failed release became
# "failed, and the rollback also failed. needs a person" — the release was
# recoverable and the recovery was not.
#
# They are compiler-facing output, regenerated by `next build` a few lines
# down, and the running server never reads them. Clearing them makes the type
# check see the checked-out source and nothing else, which is the only thing
# it was ever meant to be checking.
rm -rf .next/types .next-build/types
